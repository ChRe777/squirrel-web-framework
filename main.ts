...
import { join, fileExists, makeContext } from './utils/mod.ts';
...
async function getTypeOfPath(path: string): Promise<[string, string]> {
    ...
    if (path == "/") {
        ...
        // "index.page"
        const indexPath = "./" + join(Constants.SRC_DIR, Constants.PAGES_DIR, Constants.PAGE_INDEX);
        ...
        // "home.page"
        const homePath = "./" + join(Constants.SRC_DIR, Constants.PAGES_DIR, `home.page`);
