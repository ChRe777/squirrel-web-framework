...
async function getTypeOfPath(path: string): Promise<[string, string]> {
    ...
    if (path == "/") {
        // "index.page"
        const indexPath = "./" +
            join(Constants.SERVER_DIR, Constants.PAGES_DIR, Constants.PAGE_INDEX);
        ...
        // "home.page"
        const homePath = "./" +
            join(Constants.SERVER_DIR, Constants.PAGES_DIR, `home.page`);
    ...
    if (type == Constants.NOT_FOUND) {
        const filePath = "./" +
            join(Constants.SERVER_DIR, Constants.PAGES_DIR, `${path}.page`);
